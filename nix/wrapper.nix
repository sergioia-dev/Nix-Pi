{ pkgs, basePi, settingsJson, allExtensions, configStampValue, modelsJsonContent, keybindingsJsonContent }:
...
PI_KEYS_EOF
  ...
  # Copy ai-skills from project source to agent dir
  if [ -d "$(pwd)/ai-skills" ]; then
    cp -r "$(pwd)/ai-skills"/. "$AI_SKILLS_AGENT/" 2>/dev/null
    # Remove .git files and dirs from submodules
    find "$AI_SKILLS_AGENT" -name '.git' -type f -delete 2>/dev/null
    find "$AI_SKILLS_AGENT" -name '.git' -type d -exec rm -rf {} + 2>/dev/null
  fi

  # Update settings.json to include ai-skills path
  if [ -L "$PI_AGENT_DIR/settings.json" ]; then
    # Symlink to read-only Nix store - replace with writable copy
    SETTINGS_TARGET=$(readlink "$PI_AGENT_DIR/settings.json")
    SETTINGS_DIR=$(dirname "$PI_AGENT_DIR/settings.json")
    SETTINGS_TMP=$(mktemp "$SETTINGS_DIR/.settings.json.XXXXXX")
    cp "$SETTINGS_TARGET" "$SETTINGS_TMP"
    rm "$PI_AGENT_DIR/settings.json"
    mv "$SETTINGS_TMP" "$PI_AGENT_DIR/settings.json"
  fi

  if [ -f "$PI_AGENT_DIR/settings.json" ]; then
    node -e "
const fs = require('fs');
const path = '$PI_AGENT_DIR/settings.json';
try {
  let data = {};
  ...
  const ai_path = '$AI_SKILLS_AGENT';
  if (!data.skills.includes(ai_path)) {
    data.skills.push(ai_path);
